 ...
 * comparing IFS files in the <i>iSphere Synchronize IFS Files</i> editor.
 */
public abstract class AbstractCompareStreamFilesJob {
    ...
    /**
     * Produces a new compare IFS files job.
     * 
     * @param monitor - progress monitor
     * @param sharedValues - values shared between compare jobs
     */
    public AbstractCompareStreamFilesJob(SubMonitor monitor, CompareStreamFilesSharedJobValues sharedValues) {
    ...
    protected CompareStreamFilesSharedJobValues getSharedValues() {
        return sharedValues;
    }
    ...
    protected void cancel(int handle, Exception e) {
        ...
        ISpherePlugin.logError("*** Resolving IFS files: Unexpected error. ***", e);
    ...
    protected void cancelHostJob(int handle) {
        ...
        try {
            preparedStatementUpdate = getJdbcConnection()
                .prepareStatement("UPDATE " + sqlHelper.getObjectName(getISphereLibrary(), "SYNCIFSS") + " SET XSCNL = '*YES' WHERE XSHDL = ?");
            preparedStatementUpdate.setInt(1, handle);
            preparedStatementUpdate.executeUpdate();
        } catch (SQLException e) {
            ISpherePlugin.logError("*** Could not cancel host job of load synchronize IFS files job ***", e);
        }
