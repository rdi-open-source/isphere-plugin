...
public class SearchResultMessageIdCallback extends AbstractCallbackHandler<SearchResultMessageId> {
    ...
    public void doEndElement(Object userData, String path, int level, String elementData, XmlParser xmlParser) throws Exception {
        ...
        if ("/messageId".equals(path)) {
            getUserData().setMessageId(xmlToString(elementData));
        } else if ("/message".equals(path)) {
            getUserData().setMessage(xmlToString(elementData));
        }
    }
}
