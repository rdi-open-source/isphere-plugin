...
public class XMLSearchHelper extends AbstractXmlSearchHelper {
    ...
    private static final String SOURCE_FILE_SEARCH = "sourceFileSearch";
    ...
    public static void saveSearchResultToXML(File toFile, SearchResultTabFolder searchResultTabFolder) throws Exception {
        ...
        streamWriter.writeStartElement(SOURCE_FILE_SEARCH);
    ...
    private static void writeSearchArguments(XMLPrettyPrintWriter streamWriter, List<SearchArgument> searchArguments) throws XMLStreamException {
        ...
        streamWriter.writeAttribute("class", "linked-list");
    ...
    private static void writeSearchResult(XMLPrettyPrintWriter streamWriter, SearchResult searchResult) throws XMLStreamException {
        ...
        streamWriter.writeStartElement(MESSAGE_FILE);

        createNode(streamWriter, "connectionName", searchResult.getConnectionName());
        createNode(streamWriter, "library", searchResult.getLibrary());
        createNode(streamWriter, "messageFile", searchResult.getMessageFile());
        createNode(streamWriter, "description", searchResult.getDescription());

        writeSearchResultStatements(streamWriter, searchResult.getMessageIds());

        streamWriter.writeEndElement();

    }

    private static void writeSearchResultStatements(XMLPrettyPrintWriter streamWriter, SearchResultMessageId[] messageIDs) throws XMLStreamException {

        streamWriter.writeStartElement(MESSAGE_IDS);

        for (SearchResultMessageId searchResultMessageId : messageIDs) {
            writeSearchResultMessageId(streamWriter, searchResultMessageId);
        }

        streamWriter.writeEndElement();
    }

    private static void writeSearchResultMessageId(XMLPrettyPrintWriter streamWriter, SearchResultMessageId searchResultMessageId)
        throws XMLStreamException {

        streamWriter.writeStartElement(MESSAGE_ID);

        createNode(streamWriter, "messageId", searchResultMessageId.getMessageId());
        createNode(streamWriter, "message", searchResultMessageId.getMessage());
