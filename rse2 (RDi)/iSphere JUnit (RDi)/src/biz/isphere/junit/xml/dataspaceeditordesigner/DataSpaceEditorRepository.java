...
public final class DataSpaceEditorRepository {
     ...
     */
    private DataSpaceEditorRepository(String xmlPath) {
        repositoryLocation = xmlPath;
     ...
     */
    public synchronized static DataSpaceEditorRepository getInstance(String xmlPath) {
        if (instance == null) {
            instance = new DataSpaceEditorRepository(xmlPath);
        }
        return instance;
    }
    ...
    private String getFileName(DEditor dEditor) {
        // return getRepositoryLocation() + dEditor.getKey() + "." +
        // FILE_EXTENSION;
    ...
    private Map<String, DEditor> getOrLoadEditors() {
        ...
        if (dEditors.size() == 0) {
            ...
            //
            // Boolean createExampleDataSpaceEditorsConfirmed =
            // DoNotAskMeAgainDialog.openConfirm(UIHelper.getActiveShell(),
            // DoNotAskMeAgain.CONFIRM_CREATE_EXAMPLE_DATA_SPACE_EDITORS,
            // Messages.Create_example_data_space_editors);
            //
    ...
    // Map<String, DEditor> dExampleEditors =
    // loadEditors(getExamplesLocation());
    ...
    // try {
    // URL resourceUrl =
    // getClass().getClassLoader().getResource("sample_dataSpaceEditors");
    // return FileLocator.toFileURL(resourceUrl).getPath();
    // } catch (Exception e) {
    // ISpherePlugin.logError("*** Failed getting location of data space example
    // editors ***", e); //$NON-NLS-1$
    // }
    //
    // return null;
