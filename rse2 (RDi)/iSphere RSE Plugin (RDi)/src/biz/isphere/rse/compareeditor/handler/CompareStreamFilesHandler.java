...
public class CompareStreamFilesHandler extends AbstractCommandHandler {
    ...
    public void handleSourceCompare(StreamFile[] selectedStreamFiles, SourceMemberCompareEditorConfiguration cc) {

        if (cc.isShowDialog()) {
            if (cc.isEditMode()) {
                handleSourceCompare(selectedStreamFiles);
            } else {
                handleReadOnlySourceCompare(selectedStreamFiles);
            }
        } else {
            handleSourceCompareWithoutDialog(selectedStreamFiles, cc);
        }
    }

    private void handleSourceCompareWithoutDialog(StreamFile[] streamFiles, CompareEditorConfiguration cc) {

        // TODO: synchronize IFS files
        // cc.setDropSequenceNumbersAndDateFields(!hasSequenceNumbersAndDateFields(streamFiles));

        StreamFile leftStreamFile = streamFiles[0];
        StreamFile rightStreamFile = streamFiles[1];
        CompareStreamFileAction action = new CompareStreamFileAction(cc, null, leftStreamFile, rightStreamFile, null);
        action.run();
    }
